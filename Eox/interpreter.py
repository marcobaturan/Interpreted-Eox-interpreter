...
from Eox import expressions, statements
from Eox.tokens import TokenType, Token


class EoxRuntimeError(RuntimeError):
...
class Interpreter(expressions.ExprVisitor, statements.StmtVisitor):
    ...
    def check_number_operand(self, operator: Token, operand: Any) -> None:
        ...
        raise EoxRuntimeError(operator, 'Operando devas esti nombra objekto.')

    def check_number_operands(self, operator: Token, left: Any, right: Any) -> None:
        if self.is_number(left) and self.is_number(right):
            return

        raise EoxRuntimeError(operator, 'Operandoj devas esti nombraj objektoj.')
    ...
    def visit_binary_expr(self, expr: expressions.Binary) -> Any:
        ...
        if token_type == TokenType.BANG_EQUAL:
            ...
            raise EoxRuntimeError(expr.operator,
                                  'Operandoj devas esti du cxenoj aux du nombraj objektoj.')
