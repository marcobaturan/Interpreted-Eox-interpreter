...
class Parser:
    ...
    def next(self) -> Token:
        if self.is_at_end():
            raise EOFError('Fino de dosiero, ne povas ricevi la sekvan jxetonon.')
    ...
    def print_statement(self) -> statements.Stmt:
        ...
        self.consume(TokenType.SEMICOLON, "Atendu ';' post valoro.")

        return statements.Print(value)

    def expression_statement(self) -> statements.Stmt:
        expr = self.expression()
        self.consume(TokenType.SEMICOLON, "Atendu ';' post esprimo.")
    ...
    def primary(self) -> expressions.Expr:
        ...
        if self.match(TokenType.LEFT_PAREN):
            expr = self.expression()
            self.consume(TokenType.RIGHT_PAREN, "Atendu ')' post esprimo.")
            return expressions.Grouping(expr)

        raise self.error(self.peek(), 'Atendu esprimon.')
